...
void OccupyControl::ChangeMap(OccupyMap* Poi)//传入的应该是类型为OccypuMap类型的指针 
{
    ...
    OccupyDead* Tem=static_cast<OccupyDead*>(Poi);
    LimitedAdd(Tem);
}//完 
...
void OccupyControl::LimitedAdd(OccupyDead* Poi)//已完，未测试 
{
    int x1,z1,x2,z2;
...
void OccupyControl::DeadAdd(OccupyDead* Poi)//直接从LIMITEDADD复制 
{
    int x1,z1,x2,z2;
    std::vector<bool> Tank(BlockZ*BlockX,false);
    for(std::list<OccupyData>::iterator P=Poi->ListXZ.begin();P->Mode!=OCCPEND&&P!=Poi->ListXZ.end();)
...
bool OccupyControl::DeadCheck(float x,float y,float z,float dis)//在里面表示为TRUE 直接从原版复制，其正确性未知。 
{
    ...
    if(!XZDead.empty())
    {
        ...
        if(xl>=1&&zl>=1)
        {
            std::list<OccupyData>::iterator DP= * ((XZEnsample.begin()+(xl-1)*BlockZ+(zl-1))->PD) ;
            if(DP->Mode==OCCMARK)
            {
                float Tex=0.0;
                float Tez=0.0;
